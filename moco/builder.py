...
class MoCo(nn.Module):
    ...
    """
    def __init__(self, base_encoder, class_num=10, dim=128, K=65536, m=0.999, T=0.07, mlp=True, cluster=True):
        """
    ...
    def forward(self, im_q, im_k):
        ...
        with torch.no_grad():  # no gradient to keys
            ...
            im_k, idx_unshuffle = self._batch_shuffle_ddp(im_k)
            k = self.encoder_k(im_k)  # keys: NxC
            z_j = nn.functional.normalize(self.encoder_k.fc(k), dim=1)

            # c_j = self.encoder_k.cluster(k)

            # undo shuffle
            z_j = self._batch_unshuffle_ddp(z_j, idx_unshuffle)

        k = self.queue.clone().detach()
        self._dequeue_and_enqueue(z_j)

        # return z_i, z_j, k, c_i, c_j
        return z_i, z_j, k
    ...
    def forward_cluster(self, im_q):
        ...
        """
        # compute query features
        q = self.encoder_k(im_q)  # queries: Nx512
    ...
    def forward_cluster_z(self, im_q):
        ...
        """
        # compute query features
        q = self.encoder_k(im_q)  # queries: Nx526
        z_i = nn.functional.normalize(self.encoder_q.fc(q), dim=1)
        return z_i
