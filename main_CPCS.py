...
def main():
    ...
    parser.add_argument('--gpu', default=1, type=int,
                        help='GPU id to use.')
    config = yaml_config_hook("config/main_CPCS.yaml")
    ...
    if args.dataset == "CIFAR-10":
        ...
        dataset = torchvision.datasets.ImageFolder(
            root=dataset_dir,
            transform=transform.Transforms_for2(s=0.5, size=args.image_size, strong=True),
        )
        class_num = 200
        # class_num = 150
    else:
    ...
    model = tools.build_stage2.MoCo(
        base_model, class_num,
        args.moco_dim, args.moco_t, args.mlp, cluster=True)
    print(model)
    print(f'-----------------------------------\n{args.save_path}')
...
def train(train_loader, model, criterion_instance, criterion_ins_cent, criterion_clu_cent, criterion_cluster, optimizer, epoch, clu_cent, cent_NN, args):
    ...
    for i, (images, _) in enumerate(train_loader):
        ...
        _, c = c_i.size()
        # print(logits_cent.size())
        # logits_cent 对应的聚类中心的位置为batch数
        # cluster_label = cul_label_nn(logits_cent, c_nn.detach(), args)
        cluster_label = cul_label_nn_so(logits_cent, c_nn.detach(), args)
        ...
        # flag1 = torch.randint(low=0, high=10, size=(1,))
        # if flag1 >= 5:
        #     gaosi_random = (0.1 ** 0.5) * torch.randn(size=c_cent.size())
        #     c_cent = c_cent + gaosi_random.to(args.gpu).half()
        #     c_cent = nn.functional.normalize(c_cent, dim=1)
        # if flag1 >= 5:
        #     gaosi_random = (0.1 ** 0.5) * torch.randn(size=c_nn.size())
        #     c_nn = c_nn + gaosi_random.to(args.gpu).half()
        #     c_nn = nn.functional.normalize(c_nn, dim=1)

        # 对c_cent加上熵的损失
        p_i = c_cent.sum(0).view(-1)
        p_i /= p_i.sum()

        # 对c_cent加上熵的损失
        p_nn = c_nn.sum(0).view(-1)
        p_nn /= p_nn.sum()
        ne_i = math.log(p_i.size(0)) + (p_i * torch.log(p_i)).sum()
        ne_nn = math.log(p_nn.size(0)) + (p_nn * torch.log(p_nn)).sum()



        loss = loss_1 + loss_2 + loss_4 + loss_ins_2 + loss_cluster + loss_CE + ne_i + ne_nn
        # loss = loss_ins_2 + loss_cluister + loss_CE

        loss_cent.update(loss_cluster.item(), images[0].size(0))
        loss_aug1.update(loss_CE.item(), images[0].size(0))

        loss_orl.update(ne_i.item(), images[0].size(0))
        loss_batch.update(ne_nn.item(), images[0].size(0))
