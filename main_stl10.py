...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.reload:
        model_fp = os.path.join(args.model_path, "checkpoint_0{}.pth.tar".format(args.resume))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        loss_epoch = train(cluster_data_loader, instance_data_loader, model, criterion, criterion_instance, criterion_cluster, optimizer, epoch, args)
        # loss_epoch_ins = train(instance_data_loader, model, criterion, criterion_instance, criterion_cluster, optimizer,
        #                    epoch, args)
        ...
        print(f"Epoch [{epoch}/{args.epochs}]\t Loss: {loss_epoch / (len(cluster_data_loader) + len(instance_data_loader))}\t time: {data_time}")
...
def train(train_loader, unlabed_train_loader, model, criterion, criterion_instance, criterion_cluster, optimizer, epoch, args):
    ...
    for i, (images, _) in enumerate(unlabed_train_loader):
