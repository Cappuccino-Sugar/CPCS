...
def main():
    ...
    parser.add_argument('--gpu', default=0, type=int,
                        help='GPU id to use.')
    config = yaml_config_hook("config/one_cluster.yaml")
...
#
    ...
    ARI = []
    ACC = []
    while args.resume <= args.epochs:
        nmi_ = []
        ari_ = []
        acc_ = []
        for i in range(1):
            # assert(args.resume % 20 == 0)
    ...
    data_acc = pd.DataFrame(ACC, columns=None)
    data_nmi = pd.DataFrame(NMI, columns=None)
    data_ari = pd.DataFrame(ARI, columns=None)

    # writeFile(data_acc, data_nmi, data_ari, 'result/{}/'.format(args.dataset), 'one1000_20')
...
def validate(val_loader, model, args):
    ...
    for i, (images, target) in enumerate(val_loader):
        ...
        # compute output
        with torch.no_grad():
            output = model.forward_cluster(images)
...
def writeFile(data_acc, data_nmi, data_ari, path, filename):
    """
    ...
    if not os.path.exists(path):
        os.makedirs(path)
    writer = pd.ExcelWriter(path + filename + '.xlsx')  # 写入Excel文件
    data_acc.to_excel(writer, 'page_acc', float_format='%.5f', header=False, index=False)  # ‘page_1’是写入excel的sheet名
    data_nmi.to_excel(writer, 'page_nmi', float_format='%.5f', header=False, index=False)
    data_ari.to_excel(writer, 'page_ari', float_format='%.5f', header=False, index=False)
    writer.save()
    writer.close()
