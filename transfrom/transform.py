...
class Transforms_for2:
    def __init__(self, size, s=1.0, mean=None, std=None, blur=False, strong=False):
        self.train_transform = [
            torchvision.transforms.RandomResizedCrop(size=size),
            torchvision.transforms.RandomHorizontalFlip(),
            torchvision.transforms.RandomApply([torchvision.transforms.ColorJitter(0.8 * s, 0.8 * s, 0.8 * s, 0.2 * s)],
    ...
    def __call__(self, x):
        # return self.test_transform(x), self.train_transform(x), self.train_transform(x)
        return self.test_transform(x), self.strong_transform(x)
        # return self.train_transform(x)
