...
class clu_cent(nn.Module):
    ...
    def forward(self, z_cent, z_nn):
        ...
        c, n = z_cent.size()
        k, n = z_nn.size()
        mask_pos = self.construct_mask(c, k).detach().cuda()
        mask_one = torch.ones_like(mask_pos).detach().cuda()
...
class ins_U_cluloss(nn.Module):
    ...
    def construct_mask(self, c, b, k, max_center):
        mask_cent = torch.zeros([b, c]).cuda()
        mask_nn = torch.zeros([b, k]).cuda()
        d = int(k / c)
        replace = torch.ones(d).cuda()
        replace_one = torch.ones(1).cuda()
        flag = 0
        for i in max_center:
            index = torch.arange(int(i) * d, (int(i) + 1) * d).cuda()
    ...
    def forward(self, z_cent, z_orl, z_nn, z_i):
        ...
        b, n = z_i.size()
        k, n = z_nn.size()
        mask_pos = self.construct_mask(c, b, k, max_center).detach().cuda()
