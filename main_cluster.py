...
def main_worker(gpu, ngpus_per_node, args):
    ...
    Y_ = k.predict(X)
    print('K-means Time Elapsed: {:2.2f} seconds'.format(time.time() - start))

    start = time.time()
    c, num_clust, req_c = FINCH(X, initial_rank=None, req_clust=class_num, distance='cosine', ensure_early_exit=True,
                                verbose=True)
    print('FINCH Time Elapsed: {:2.2f} seconds'.format(time.time() - start))



    # zero = torch.zeros([19500, 15])
    # Y_ = k.predict(X)
    # l = np.int64(Y_)
    # print(l)
    ...
    print('K-means NMI = {:.4f} ARI = {:.4f} F = {:.4f} ACC = {:.4f}'.format(nmi, ari, f, acc))

    nmi, ari, f, acc = evaluation.evaluate(Y, req_c)
    print('FINCH NMI = {:.4f} ARI = {:.4f} F = {:.4f} ACC = {:.4f}'.format(nmi, ari, f, acc))
