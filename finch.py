...
def main():
    ...
    name = "CIFAR-10"
    # name = 'MNIST'
    data_fea = os.path.join(source_dir, '{}/{}_Feature_{}.npy'.format(name, 'stage2', 200))
    data_lab = os.path.join(source_dir, '{}/{}_Ytrue_{}.npy'.format(name, 'stage2', 200))
    data = np.load(data_fea).astype(np.float32)
    Y = np.load(data_lab)
    num_class = 10
    start = time.time()
    c, num_clust, req_c = FINCH(data, initial_rank=None, req_clust=num_class, distance='cosine', ensure_early_exit=True, verbose=True)
